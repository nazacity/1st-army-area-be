...
export class UnitController {
  ...
  async getAll(
    ...
    try {
      ...
      const { data, total } = await this.unitService.getAllUnits(query, adminUnitIds)
  ...
  async create(
    ...
    try {
      if (req.user.units?.length > 0) {
        throw new HttpException({ message: 'ไม่มีสิทธิ์จัดการหน่วย' }, HttpStatus.FORBIDDEN)
      }
      const adminId = req.user?.id
      const data = await this.unitService.createUnit(dto, adminId)
      return { data }
    } catch (error) {
      throw new HttpException({ message: error.message }, error.status ?? HttpStatus.BAD_REQUEST)
    }
  }
  ...
  async update(
    ...
    try {
      ...
      const adminId = req.user?.id
      const data = await this.unitService.updateUnit({ id, update: dto, adminId })
      return { data }
    } catch (error) {
      throw new HttpException({ message: error.message }, error.status ?? HttpStatus.BAD_REQUEST)
    }

...
**`จำพวก (Type)` — 9 ค่า unique → map เข้า enum 8 ค่า (English keys, แสดงผลด้วย `PERSONNEL_TYPE_LABELS` ไทย):**

| ค่าใน CSV | จำนวน | Map ไป |
|---|---|---|
| `ทบ.` | 122 | `ARMY` (ทบ.) |
| `สป.` | 10 | `MOD` (กลาโหม) — สป. = สำนักงานปลัดกระทรวงกลาโหม/ส่วนกลางกลาโหม (สป.กลาโหม, กรมการสรรพกำลังกลาโหม, กรมเสมียนตรา, กรมพระธรรมนูญ) — สังกัดกระทรวงกลาโหม ไม่ใช่ ทบ. |
| `ทบ., นักบิน` | 4 | `ARMY` (นักบิน ทบ.) |
| `นักบิน` | 3 | `ARMY` — สังกัด กองพันบินที่ 1/2/3 กรมบิน, ศูนย์การบินทหารบก = หน่วยบินทหารบกทั้งหมด (ไม่ใช่ ทอ.) |
...
| `ทบ., ฉก.ทม.รอ.` | 1 | ผู้กรอกติ๊กคร่อม 2 ช่อง (ฟอร์มมีจริงแค่ `ทบ.` หรือ `ฉก.ทม.รอ.`) — normalize ก่อน map: ค่าใดมี `ฉก.` อยู่ในข้อความ → ใช้ case `ROYAL_PAGE_GUARD` + `is_special_forces = true` (เลข 105210) |

> **หมายเหตุ ฉก.**: `ฉก.ทม.รอ.` (ทหารมหาดเล็กราชวัลลภรักษาพระองค์) เป็นจำพวกของตัวเองใน enum (`ROYAL_PAGE_GUARD`) ส่วน field `is_special_forces` ยังเก็บคู่ไว้เผื่อ filter ตัวอย่าง: type `ROYAL_PAGE_GUARD` + isSpecialForces `true`
...
const TYPE_MAP: Record<string, PersonnelType> = {
  ...
  'สป.': PersonnelType.MOD,            // ส่วนกลางกลาโหม (สำนักงานปลัดกระทรวงกลาโหม)
  'ทบ., นักบิน': PersonnelType.ARMY,   // กองพันบิน กรมบิน ทบ.
  'นักบิน': PersonnelType.ARMY,        // กรมบิน/ศูนย์การบินทหารบก = ทบ. (ไม่ใช่ ทอ.)
  'บก.ทท.': PersonnelType.JOINT_FORCE, // กองบัญชาการกองทัพไทย (joint) — ไม่ใช่ ทอ.
  'มิตรประเทศ': PersonnelType.FOREIGN,
  'ฉก.ทม.รอ.': PersonnelType.ROYAL_PAGE_GUARD, // ทหารมหาดเล็กราชวัลลภรักษาพระองค์
};
...
function inferType(branch: string, unit: string): PersonnelType {
  ...
  return PersonnelType.ARMY; // default: ทบ. (รวม กรมบิน/ศูนย์การบินทหารบก)
}

// ค่าผสม/ค่าไม่ตรง map ที่มี 'ฉก.' → ROYAL_PAGE_GUARD
function refineType(type: PersonnelType, sourceTypeRaw: string): PersonnelType {
  if (sourceTypeRaw.includes('ฉก.')) return PersonnelType.ROYAL_PAGE_GUARD;
  return type;
}
...
1. **mapping `จำพวก` ตัดสินแล้วด้วยการวิเคราะห์เหล่า+สังกัดเดิม** (§1.2): นักบิน → ARMY, สป. → MOD (กลาโหม), บก.ทท. → JOINT_FORCE, ฉก.ทม.รอ. → ROYAL_PAGE_GUARD (ทหารมหาดเล็กราชวัลลภรักษาพระองค์), มิตรเหล่า → inferType() รายคน (AIR_FORCE/NAVY/POLICE) — เก็บ `source_type_raw` ไว้ทั้งหมด แอดมินแก้เป็นรายคนได้ภายหลัง
